...
namespace Pianoforte.Sena.Lang.Test
{
  public class LexerTest
  {
    ...
    public static object[][] lexData = {
      ...
      new object[] { "+-*/+=-=*=/===!==<><=>=()[]\n", new[] {
        new Token(TokenKind.OpPlus, "+", new TokenPosition("", 1, 1)),
        new Token(TokenKind.OpMinus, "-", new TokenPosition("", 1, 2)),
        new Token(TokenKind.OpMultiplication, "*", new TokenPosition("", 1, 3)),
        new Token(TokenKind.OpDivision, "/", new TokenPosition("", 1, 4)),
        new Token(TokenKind.OpPlusAssignment, "+=", new TokenPosition("", 1, 5)),
        new Token(TokenKind.OpMinusAssignment, "-=", new TokenPosition("", 1, 7)),
        new Token(TokenKind.OpMultiplicationAssignment, "*=", new TokenPosition("", 1, 9)),
        new Token(TokenKind.OpDivisionAssignment, "/=", new TokenPosition("", 1, 11)),
        new Token(TokenKind.OpEqual, "==", new TokenPosition("", 1, 13)),
        new Token(TokenKind.OpNotEqual, "!=", new TokenPosition("", 1, 15)),
        new Token(TokenKind.OpAssignment, "=", new TokenPosition("", 1, 17)),
